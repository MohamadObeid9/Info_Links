...
func TestHandleAdminGetAnalyticsSummary(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			errMsg:       "Range must be 7, 30, 90, or all",
			wantRange:    "5",
		},
		{
...
func TestHandleAdminGetAnalyticsActors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			statusWanted: http.StatusBadRequest,
			errMsg:       "kind must be link, extra_link, course, extra_section, service, favorite, search, or device",
			wantKind:     "widget",
			wantID:       "1",
			wantRange:    "7",
		},
		...
		},
		{
			...
			statusWanted: http.StatusBadRequest,
			errMsg:       "Range must be today, 7, 30, 90, or all",
			wantKind:     "link",
			wantID:       "1",
			wantRange:    "5",
		},
