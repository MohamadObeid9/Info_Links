...
- **Guest rows accumulate.** Every visitor who never registers leaves an unclaimed row behind. A cleanup job for stale guests is future work; until then the table grows with traffic and `users` row count is not the same as student count (filter on `is_guest = false`).
